                ...
                <div class="col-sm-12 col-md-12">
                    <div class="box">
                                    ...
                                    <div class="col-md col-sm col-lg">
                                        ...
                                        <select class="form-control" id="danaSelect" name="danaSelect">
                                            @foreach ($dana as $item)
                                                <option value="{{ $item->id }}" {{ $danaSelect == $item->id ? 'selected' : '' }}>{{ $item->nama_sumber_dana }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div class="box-body">
                            ...
                            @endif
                            <form action="{{ url('/operator/realisasi/store') }}" method="POST">
                                @csrf
                                <div class="table-responsive">
                                    ...
                                    <div class="col">
                                        @if (Auth()->user()->isAdmin != 1)
                                            @if ($aktif->status == '1')
                                                <button type="submit" class="md-btn md-raised m-b-sm w-xs blue StoreReport"
                                                    role="button">SIMPAN
                                                </button>
                                            @else
    ...
    <script>
        ...
        $(document).ready(function() {
            $("#bulanSelect").change(function() {
                ...
                window.location.href = '{{ url('/operator/realisasi') }}?bulan=' +
                    bulan + '&dana=' + dana;
            });
        });
        $(document).ready(function() {
