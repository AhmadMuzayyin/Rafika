...
class AdminDashboardController extends Controller
{
    ...
    {
        $data = SubKegiatan::where('pak_id', session()->get('pak_id'))->get();
        $jml_sub = SubKegiatan::where('pak_id', session()->get('pak_id'))->get()->count();
        $jml_dana = SumberDana::all();
        $jml_pengadaan = Pengadaan::all();
        $report = SubKegiatan::join('jadwals', 'sub_kegiatans.id', '=', 'jadwals.sub_kegiatan_id')->where('sub_kegiatans.pak_id', session()->get('pak_id'))->where('jadwals.pelaksanaan', session()->get('pelaksanaan'))->where('jadwals.bulan_id', now()->format('n'))->where('jadwals.status', 1)->get();
        $jml_sub_lapor = 0;
